...
__global__ void computeNCountGrams(int *d_lengths, int *d_bins, int bins_size, int M, int N)
{
    ...
    //initialization
    for(i=threadIdx.x; i<bins_size; i+=blockDim.x)
        shared_bins[i] = 0;
    __syncthreads();
    ...
    if(id < M-N+1)
    {
        ...
        atomicAdd(shared_bins+bid, 1);
    }
    __syncthreads();

    //update global bins
    for(i=threadIdx.x; i<bins_size; i+=blockDim.x) //uses many threads
        atomicAdd(d_bins+i, shared_bins[i]);
    // if(threadIdx.x == 0)  // uses one representative thread
...
int main(int argc, char *argv[])
{
    ...
    for(int i=10; i>=5; i--)
    {
        ...
        for(int j=0; j<20; j++)
        {
            ...
            computeNCountGrams<<<(M+N-1 +(1<<i)-1)/(1<<i), (1<<i), sizeof(int)*bins_size>>>(d_lengths, d_bins, bins_size, M, N);
    ...
    FILE* fp = fopen("cs16b107_out_3.txt","w");
